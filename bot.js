...
function handleSaveToDB(agent){
  agent.add("hello");
  console.log("ADADS");
  return admin.database().ref('a/b').once("value").then((snapshot) => {
    var a = snapshot.child('c').val();
    agent.add(new Card({
      title: "ye " + a,
      text: 'This is the body text',
      buttonText: 'hi',
      buttonUrl: 'https://console.dialogflow.com/api-client/#/agent/36489b37-222d-49e6-8671-7c8488dabb20/fulfillment'
    })
              );
  });
}
  ...
  return admin.database().ref('financial-advisor-ftvfvy').once("value").then((snapshot) => {
    snapshot.forEach(function(child) {
      ...
      if(check(JSON.stringify(d).slice(1, 11), startdate, enddate)){
         agent.add(new Card({
          	title: child.child('d').child('name'),
      	text: 'amount: ' + JSON.stringify(child.child('d').child('amount'))+ '\n  \n' + 'date:' + JSON.stringify(child.child('d').child('date'))
